...
1. **ADP:** owner does *not* hand-maintain ADP. The tool pulls live market ADP automatically — see §8a (FFC public ADP API, verified live; per-format snapshots incl. superflex; daily cron; upload-column precedence; UI attribution).
   **REVISED 2026-08-19 — presets are no longer owner-authored either.** The free boards are auto-derived from those same FFC snapshots on every nightly run: four "Consensus" boards (PPR / Half-PPR / Standard / Superflex), 400 players each (ADP-covered pool + a Sleeper-popularity tail so 20-team leagues don't run dry), refreshed in place. Nothing about launch waits on owner content; owner-authored boards remain an optional differentiator via the admin publish path. Consequence: a board derived *from* market ADP can't be judged *against* it, so derived boards suppress the value/reach cell colouring and the "going ~N picks later than his rank" reason (`RankingSet.derivedFrom` → snapshot `adpDerived`). Everything measuring ADP against the pick number — survival, outlook, scarcity, VBD — is unaffected, and all of it lights up on user uploads, which is the paid-tier hook. FTN-credited legacy data remains test-fixtures-only.
